class ReceiptsController < ApplicationController
  ...
  def my_receipts
    if User.ids.include?(params[:id].to_i) && (((User.find(params[:id].to_i).id == current_user.id) || (current_user.role == 3)))
  ...
  def show
    ...
    if Receipt.ids.include?(params[:id].to_i)
       ...
       if (current_user.role == 3) || (@receipt.user_id == current_user.id)
